...
```{r}
MD_COVID_19_Cases_by_County<-read_csv("data/MD_Cases_By_County.csv")
```
...
head(Clean_MD_Cases_By_County)
```

Was having a slight issue with dates being NA after my intial mutate, worked trough the issue with ChatGPT and ended up here. Literally copied the format of the date column with my code and asked, "Why did this come up NA"

```{r}
Clean_MD_Cases_By_County <- Clean_MD_Cases_By_County %>%
  mutate(date = mdy_hms(date),  # Convert character to date
         date = as.Date(date))  # Extract only the date part

# Now the 'date' column will contain the date part only

Clean_MD_Cases_By_County

```

This made my date columns NA, I showed my original format in ChatGPT and asked why it became NA and got this code.
...
```{r}
MD_COVID_19_Vaccinations_by_County <- read_csv("data/MD_Vaccinations_By_County.csv")
```
...
Population_Fully_Vaccinated<-County_Pop_Vaccinated %>% 
  mutate(Pop_Percent_Fully_Vaccinated = fullyvaccinatedcumulative/estimate*100)
...
```{r}
MD_COVID_19_Probable_Deaths_by_County <-read_csv("data/MD_COVID_19_Probable_Deaths_by_County.csv")
```
