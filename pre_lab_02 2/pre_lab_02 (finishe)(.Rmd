...
**Answer** It is important because it decides what is put in our frame and makes the data significantly easier to navigate.By choosing that function with the specific journalism courses, it filtered to those specifically.
...
**Answer** Using the or operator gives us 41,011, a significantly bigger number. 
...
**Answer:** Due to the mutation, instead of operating as a CHR, its operating as a date. Hence, the number is now in year, month, day, which is how most other nations operate. 
...
**Answer** In the new month column, each month is listed as year,month,01, essentially meaning it is adding up the total from the start of the month to the final day of it. 
...
**Answer** This code here is adding the total votes and then dividing the votes Moore got by that total number to find the answer. 
...
**Answer** The answer changed because it now put it in descending order starting with the county that voted Moore the highest. 
...
**Answer** While both answers are fine, I do think the organization on this one makes it easier to see which areas Moore dominated. 
...
**Answer** One similar command is str_yo_sentence, which formats the term like a sentence, making only the first letter capital. Also str_to_lower does the opposite of str_to_upper, instead making a term lowercase.
...
**Answer** This code is splitting up each expense by which state it originates from. It is evaluating each case and essentially filtering it by which state the address is in. 
...
1. For the Maryland expenditure, I'd like to see who Maryland is paying for services the most consistently. 

2.  For the University of Maryland classes, I'd like to see which areas are there independent studies offered in the most. I'd filter it based on school. 

3.  For the Maryland expenditures, I'd filter it by committee and see which committees are costing the most money. 
