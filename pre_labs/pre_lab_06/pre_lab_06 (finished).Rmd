...
**Task** Load the four CSV population estimate files. Do they have the same column structure? **Answer** One of the total is lowercase,other than that the structure is the same
...
**Task** Run the following code to filter on Frederick County's registration data and look at the results. What stands out to you about the changes in party registration over time? **Answer** A growth in democrat and unaffiliated people stands out to me. 
...
Left join means no matches, inner join only shows matches

**Task** Run the following code to join Maryland population data and the 2022 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer** There are 24 rows, which is the right number

```{r}
# with nrow included to show row total
maryland_population %>%  left_join(county_voters_2022, join_by("COUNTY"=="county")) %>%  nrow()
```
...
```{r}
# without nrow
maryland_population %>%  left_join(county_voters_2022, join_by("COUNTY"=="county"))
```
...
maryland_population_with_voters <- maryland_population %>% 
  left_join(county_voters_2022, join_by("COUNTY"=="county"))
```

### Task 9: Add the Demographic Data

**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **Answer** It gives total population that is of voting age and breaks it down by race. 
...
This gives demographics, such as age, party and race for each county. 
```{r}
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer** Prince George's has the highest percentage of 18+ citizens in general and the highest percent of democrats at 77.10%

```{r}
maryland_population_black_and_democrats<-maryland_population_with_voters_and_demographics %>% 
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer** There is a significantly lower asian population than black population, however the democrat population is identical. 
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N column? **Answer** It was originally a number column. 
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N? **Answer** It is a character
