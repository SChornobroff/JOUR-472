...
**Task** Export the CSV file to your pre_lab_05 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer** The headers are awkward characters, for example saying X16 or X8, instead of a desired column name. Also, I'm not sure if Baltimore City and Baltimore County should be merged into one. 
...
**Task** Load the data without headers by running the codeblock below. What problems remain? **Answer** The headers are still numbers and we have a category for all of Maryland that doesn't add up to the total. 
...
**Task** Run the codeblock below. What else do we need to do? **Answer** I would like to get rid of period in jurisdiction column and I'd also arrange it descending by total. 
