...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer**It does not show any parsing errors on this one
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer** The vendor name field is a character type and the date is also a character 
...
**Answer** 549 have none, these are mostly explained by a preceding 0 or being foreign.  
...
**Answer** A random column L and missing data stands out as problematic in my opinion. 
...
**Answer** The smallest amount is under 100 there are three under 300. There are more than 17,000 rows and the amount of organizations and even individuals who earned grants varies. 
```{r}
md_grants_loans %>% 
  arrange(Amount)
```
...
**Answer** The first line is ensuring that the clean names used before are used again. The second line is renaming the column from grantor to source. The third line is mutating the columns to uppercase. 
...
**Answer** 17,740 rows and nine columns
...
**Answer** There is a new column with a five digit zip code 
...
**Answer** The Zip Code 21202, which is a part of Baltimore. That makes sense in my opinion considering the size of the city.    

from the Maryland Tourism Board (filter, Maryland Tourism Board)
Which zip code ( group by zip 5)
most amount of money summarise (sum)
Arrange descending 
