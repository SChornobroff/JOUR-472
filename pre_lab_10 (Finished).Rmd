...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer** Below I see a large amount of text, detailing the colums. It describes there are 526 features with seven fields. The columns are OBJECTID, ZIPCODE2, ZIPNAME, EXISTING, ZIPCODE1, Shape_Leng, Shape_Area, geometry
...
**Task** Run the following code. Describe the output in the space below. **Answer** A mostly gray map of Maryland, organized by zipcode, with the longitude and latitude being the X and Y coordinates. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** It installed the library tigris. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** An 18 column chart with 3,234 rows is previewed. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** A chart with data from Maryland's 24 counties, including the geometry appears below. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** A map of Maryland divided by the 24 counties
...
**Task** Run the following code. Describe the output in the space below. **Answer**A map showing of the state, seperating by both the zip code and the counties. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** A chart with Zip Codes and the amount of foreclosures in each zip code for March 2023. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** A more in depth chart, showing the names of the towns coinciding with each zip code.
...
**Task** Run the following code. Describe the output in the space below. **Answer* Each zip code on a color coded map, demonstrating the area with the most foreclosures. The lighter the blue, the more foreclosures within the county. 
...
**Task** Run the following code. Describe the output in the space below. Be sure to input your census api key. **Answer** It loaded the necesary libraries with the census API key as well.
...
**Task** Run the following code. Describe the output in the space below. **Answer** There are two data sets. The first one (md_county_notices) gives the amount of notices for each county, while md_ounty_population gives the population per zip code. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** A chart with the zip code, county name and population of each Maryland county. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Two seperate dataframes, one with the county population and one with the notices. The notices shows up as a chart detailing the notices in each county. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The datafrane was changed to make all names capital, the column "estimate" to "Population" and the column "name" above the name of each county was altered to "county."
...
**Task** Run the following code. Describe the output in the space below. **Answer** A combined file with both the county population and its intent to foreclose notices. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** A chart arranged by rate of foreclosures per 1,000 people. Charles County has the highest rate at over 2%. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** A map with the name of each county labeled, howeer they are all in black, clashing with a lot of the dark blue colors. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The colors are no much brighter and much easier to identify which areas have the highest rate of foreclosure notices. 
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer** The wealthier areas of the county have the lowest rate of foreclosure. The southeast has a low rate of foreclosure, as well as the northwest. 
