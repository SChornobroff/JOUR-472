...
```{r}
Baltimore_OD_Mutate <- read_csv("tabula-Baltimore County; Carey, Samantha log OD.csv", col_names = FALSE) %>% 
  clean_names() %>% 
  rename(date = x1,time = x2, case = x3, evytp = x4, location = x5) %>% 
...
There are 23 calls on 7/14/22 and 10/04/22. Based on the result, I believe there are some dates with no overdose calls or they have not been 
...
```{r}
days_of_week <- Baltimore_OD_Mutate %>% 
  mutate(weekday = wday(date, label = TRUE, abbr = FALSE)) %>% 
  group_by(weekday) %>% 
  summarise(count=n (),count/4112*100) %>% 
  arrange(desc(count)
...
A3.The thing that sticks out to me most is the disproportionate amount between 4540 Silver Spring Rd and the rest of the addresses. 
...
A4.For a story idea, I think something on 4540 Silver Spring Rd and what it was would be helpful. There were also nine overdoses at Star Motel, I think that could be something worth looking into. 
