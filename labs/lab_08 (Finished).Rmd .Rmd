...
```{r}
setwd("~/Downloads/Lab 8/data")
read_csv("prince_georges_2022_overdoses.csv")
PGC_911_Calls<- read_csv("prince_georges_2022_overdoses.csv")
```

```{r}
PGC_Calls_By_Date <- PGC_911_Calls %>% 
...
December had the highest percentage of calls in general. 
...
**A1.** July, November and December, while January and February have the least. As a whole, there's a lot of correlation in the beginning and end, but the middle months have varying places. 

```{r}
PGC_Calls_With_Percent <- PGC_Calls_By_Date %>% 
  group_by(month) %>% 
  summarize(total_calls = n()) %>% 
  mutate(total_calls/1397*100)
```

```{r}
PGC_Calls_With_Percent %>% 
    ggplot() +
  geom_bar(aes(x= month, weight=total_calls/1397*100)) +
  labs(
    title="Overdose Calls Peak During Holiday Season",
    x = "month",
    y = "Percent of calls per month",
...
**A2.** There is a large fluctuation in the first 20 weeks before a spike near the 30 week mark before another drop. There's a fairly consistent growth until the last two weeks when there is another spike. 
...
PGC_Calls_By_Date %>% 
  ...
  labs(
    title="Overdose calls peak in the middle of the year",
    x = "Week",
    y = "Total Calls",
    caption = "source: PG County EMS")
...
**A3.**  For the most part, lower income areas make a higher number of 911 calls. While there are outliers, like 20710 and 20712 those are lower population incomes. To fully contextualize this, I'd try to measure this on a per capita basis to quantify it on an equal basis. 

```{r}
PGC_Calls_By_Zip<-PGC_Calls_By_Date %>% 
  group_by(zipcode) %>% 
  summarize(total_calls = n()) %>%  
...
```{r}
md_household_income <- get_acs(geography = "zcta",
              variables = c(medincome = "B19013_001"),
              state = "MD",
              year = 2019)
```
```{r}
Maryland_Household_Income <- md_household_income %>% 
  rename(zipcode = GEOID)
```

```{r}
PGC_Census_Combined <- PGC_Calls_By_Zip %>% left_join(Maryland_Household_Income, join_by("zipcode")) %>% 
...
```{r}
PGC_Census_Scatter <- ggplot(PGC_Census_Combined, aes(x = estimate, y = total_calls)) + geom_point() +
  labs(
    title="Lower income areas make more 911 calls",
    x = "Median Income",
    y = "Total Calls",
    caption = "source: PG County EMS/US Census Data") +
