...
# Load the Maryland expenditures data table
 ...
 md_expenses_clean_new <-md_expenses %>% 
  clean_names() %>% 
...
```{r}
 md_media_expenses<-md_expenses_clean_new %>% 
  filter(str_detect(expense_category, "Media")) %>% 
  filter(!is.na(payee_name))

write_csv(md_media_expenses,"data/md_media_expenses.csv")


```

```{r}
 Lab_04_Media_Expenses %>% 
...
**A2.** A number of these committees are for people running for governor. Those include:Ashwani Jain, Franchot Anderson-Walker (Franchot Gov., Anderson-Walker running mate,) Jon Baron, with both King and Siri, as well as King John for Governor have to do with John King's campaign. Peter Franchot is state comptroller, Billy Boniface ran for Harford County Executive,Eve Schuman ran for Prince George's County Council. The working families party is a group trying to combat what they see as a bigoted political system. CASA in Action is a PAC representing latino communities and immigrants as a whole. 

When looking for Jain Ashwani all three of her adds were taken down.
...
**A3.** Wes Moore is missing from that Q1 list. According to the data, Wes Moore did not run ads. SKDK is the biggest recipient of Moore's media funding.  

```{r}
Lab_04_Media_Expenses %>% 
  filter(str_detect(committee_name,"Moore  Wes")| str_detect(committee_name, "Moore Miller")) %>% 
...
**A4.** There is inherently a lot more total dollars spent, but I also noticed the democratic caucus and democratic senate caucus also spent a lot. While the younger audience, which is largely democratic, pivots to new medias, so are there politicians. I did not find any PAC's using exclusively Facebook. 
...
**A5** For what I found through Facebook, I noticed a number of Republican governor candidates, but also a number of town candidates invest heavily. We don't know however when there's a political consulting firm, if they are investing dollars into facebook. This is especially true for Wes Moore. For a story, I'd focus on different spending habits between parties. The Democrat leaning PACs and gubernatorial candidates are investing in other online media.   
